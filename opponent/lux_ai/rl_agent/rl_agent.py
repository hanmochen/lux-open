...
CHECKPOINT_PATH = os.path.dirname(__file__) + '/op_model.pt'
AGENT = None
...
class RLAgent:
    def __init__(self, obs, conf):
        ...
        self.env = env
        # print("-----------------------------------------------------------")
        # print("if this line shows up, the RLAgent.__init__ is working")
        # print("-----------------------------------------------------------")
        self.env.reset(observation_updates=obs["updates"], force=True)
        # for key_a_out in a_out.keys():
        #     # try:
        #     print(a_out[key_a_out].shape)
        #     print("the shape of a_out[{}] is {}".format(key_a_out, a_out[key_a_out].shape))
        #     # except:
            #     print("a_out[{}] is {}".format(key_a_out, a_out[key_a_out]))
        self.action_placeholder = {
            key: torch.zeros(space.shape)
            for key, space in self.unwrapped_env.action_space.get_action_space().spaces.items()
        }
        # for key in self.action_placeholder.keys():
        #     print("the shape of action_placeholder[{}] is {}".format(key, self.action_placeholder[key].shape))
    ...
    def preprocess(self, obs, conf) -> NoReturn:
        # print("the opponent step is {}".format(obs["step"]))
        # print("the opponent observation is {}".format(obs["updates"]))
        ...
        # print("opponent - the game_state.turn is {}".format(self.game_state.turn))
...
def agent(obs, conf) -> List[str]:
    global AGENT
    if AGENT is None or obs["step"] == 0:
