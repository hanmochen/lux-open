...
class RewardParser(object):
    ...
    def process(self, done, global_info):
        ...
        if done:
            ...
            # else:
            #     reward = [0,0]
            sqrt_city_diff = np.sqrt(np.abs(global_info['ct_count'][0]-global_info['ct_count'][1]))
            if global_info['ct_count'][0]>global_info['ct_count'][1]:
                reward = [sqrt_city_diff,-sqrt_city_diff]
            else:
                reward = [-sqrt_city_diff,sqrt_city_diff]
